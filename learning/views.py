...
def parent_dashboard(request):
    ...
    for link in child_links:
        ...
        lessons_done = LessonProgress.objects.filter(
            student=child,
            is_done=True,
            lesson__module__course__enrollments__student=child,
...
def lesson_detail(request, pk):
    ...
    return render(
        ...
        "learning/lesson_detail.html",
        {
            ...
            "steps": steps,
            "lesson_progress": LessonProgress.objects.filter(student=request.user, lesson=lesson).first(),
        },
    )
...
def toggle_lesson_completion(request, pk):
    ...
    lesson = get_object_or_404(Lesson.objects.select_related("module__course"), pk=pk, is_available=True)
    if not user_has_course_access(request.user, lesson.module.course):
        return redirect("dashboard")
    ...
    messages.success(request, "Урок позначено як пройдений." if progress.is_done else "Позначку про проходження уроку знято.")
